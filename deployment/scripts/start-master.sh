...
ssh $ssh_options zhe@$master_ip "
  mkdir -p $remote_code_dir &&
  mkdir -p $remote_config_dir &&
  mkdir -p $remote_exp_dir/raw-results" || exit 1

# Upload code to the master
rsync --progress -rptz -e "ssh $ssh_options" $local_code_files "zhe@$master_ip:$remote_code_dir" || exit 2

# Upload config to the master
rsync --progress -rptz -e "ssh $ssh_options" $exp_data_dir/config/* "zhe@$master_ip:$remote_config_dir" || exit 3

# Upload analysis scripts and queries to the master
rsync --progress -rptz -e "ssh $ssh_options" queries scripts "zhe@$master_ip:$remote_work_dir" || exit 4

# Upload commands to the master
scp $ssh_options "$exp_data_dir/$local_master_command_file" "zhe@$master_ip:$remote_master_command_file" || exit 5
...
ssh $ssh_options zhe@$master_ip "
  ...
  echo 'Compiling ISS.' &&
  export PATH=\$PATH:$remote_gopath/bin:$remote_work_dir/go/bin &&
  export GOPATH=$remote_work_dir/gopath
  export GOROOT=$remote_work_dir/go
  echo '$GOPATH'

...
init_command="
  ...
  cd $remote_work_dir &&
  rsync --progress -rptz -e \"ssh $ssh_options\" zhe@$master_ip:$remote_tls_directory . &&
  cd tls-data &&
  ./generate.sh $public_ip $private_ip &&

  cd $remote_work_dir &&
  rsync --progress -rptz -e \"ssh $ssh_options\" zhe@$master_ip:$remote_gopath/bin/* $remote_gopath/bin/ &&
  mkdir -p config &&

  rsync --progress -rptz -e \"ssh $ssh_options\" zhe@$master_ip:$remote_work_dir/scripts/stubborn-scp.sh $remote_work_dir/bin/ &&
...
while ! ssh $ssh_options zhe@$public_ip "$init_command"; do
  sleep 1
  echo "Retrying to initialize slave."
done

echo "Master ready. Starting slave process."
ssh $ssh_options zhe@$public_ip "$slave_command"
